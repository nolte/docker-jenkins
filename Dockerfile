...
# install python for molecule and ansible
...
RUN mkdir -p $KUSTOMIZE_PATH && \
  wget -q "https://github.com/kubernetes-sigs/kustomize/releases/download/v$(echo $KUSTOMIZE_VERSION)/kustomize_$(echo $KUSTOMIZE_VERSION)_linux_amd64" -O $KUSTOMIZE_PATH/kustomize && \
